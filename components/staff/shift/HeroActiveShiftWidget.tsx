    ...
    <div className="relative overflow-hidden rounded-3xl border border-[#9a6c37]/20 bg-gradient-to-br from-[#29241f] via-[#38312b] to-[#1f1b18] p-6 text-white shadow-xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-wider ${
          isPaused ? "bg-amber-500/20 text-amber-300 border border-amber-500/40" :
          ...
          checkInWindowInfo.isWaitingForStart ? "bg-amber-500/20 text-amber-300 border border-amber-500/40" :
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_auto]">
        ...
        {/* Live Active Work Time / Paused Timer Widget (Section 8) */}
        <div className="flex flex-col items-start justify-center rounded-2xl bg-white/5 p-4 backdrop-blur-sm lg:items-end">
          ...
          <div className={`mt-1 font-mono text-3xl font-black tracking-wider ${isPaused ? "text-amber-400" : "text-emerald-400"}`}>
            {formatTimer(isPaused ? pausedSeconds : elapsedSeconds)}
          </div>
          <div className="mt-1 flex flex-col gap-0.5 text-[10px] text-gray-400 lg:text-right">
            {isPaused ? (
              <span className="text-emerald-300/90 font-medium">Active so far: {formatTimer(elapsedSeconds)}</span>
            ) : (
              checkInTime && (
            ...
            {totalPauseMins > 0 && (
              <span className="text-amber-300/80">({totalPauseMins} mins total break logged)</span>
            )}
          </div>
        </div>
      </div>
      ...
      {!isCheckedIn && !isCompleted && !checkInWindowInfo.canCheckIn && (
        <div className="mt-4 flex items-center gap-2 rounded-xl bg-amber-500/20 p-3 text-xs text-amber-200 border border-amber-500/30">
      ...
      <div className="mt-4 border-t border-white/10 pt-4 flex flex-wrap items-center gap-3">
        {!isCheckedIn && !isCompleted && (
          <button
            ...
            className={`inline-flex min-h-12 flex-1 items-center justify-center gap-2.5 rounded-xl px-6 text-sm font-bold text-white shadow-lg transition ${
              checkInWindowInfo.canCheckIn
                ? "bg-gradient-to-r from-emerald-500 to-emerald-600 hover:brightness-110 active:scale-[0.98]"
                : "bg-gray-700/60 text-gray-400 border border-white/10 cursor-not-allowed opacity-75"
            }`}
          >
            {checkInWindowInfo.canCheckIn ? (
          ...
          <>
            ...
            {isPaused ? (
              <button
                ...
                className="inline-flex min-h-12 flex-1 items-center justify-center gap-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-emerald-600 px-5 text-sm font-bold text-white shadow-lg transition hover:brightness-110 active:scale-[0.98] disabled:opacity-60"
              >
                <Play size={18} className="fill-white" />
              ...
              <button
                ...
                disabled={actionLoading}
                className="inline-flex min-h-12 flex-1 items-center justify-center gap-2.5 rounded-xl bg-gradient-to-r from-amber-600 to-amber-700 px-5 text-sm font-bold text-white shadow-lg transition hover:brightness-110 active:scale-[0.98] disabled:opacity-60"
              >
            ...
            <button
              ...
              disabled={actionLoading}
              className="inline-flex min-h-12 flex-1 items-center justify-center gap-2.5 rounded-xl bg-gradient-to-r from-rose-500 to-rose-600 px-5 text-sm font-bold text-white shadow-lg transition hover:brightness-110 active:scale-[0.98] disabled:opacity-60"
            >
